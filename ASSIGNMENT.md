...
move on** rather than emailing us. One part of this problem (the payment _shape_)
...
- `max_terms`, `max_payments` — both cap the number of creditor payments `k`.
  ...
  `k ≤ min(max_payments, max_terms)`. _(Author note: these are currently
  redundant — consider dropping one or giving them distinct meanings.)_
...
Within the hard constraints above, **many schedules can be valid.** We do _not_
...
```json
{
  ...
  "schedule": [
    {
      "date": "2026-01-31",
      "creditor_payment_cents": 8334,
      "program_fee_cents": 11166,
      "bank_fee_cents": 500,
      "balance_cents": 0
    }
  ],
...
_(Illustrative: the row is self-consistent — `8334 + 11166 + 500 = 20000`, matching
a single \$200.00 draft with `bank_fee_cents = 500`, leaving balance 0.)_
